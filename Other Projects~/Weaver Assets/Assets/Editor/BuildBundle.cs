...
namespace WeaverAssets
{
	...
	{
		static string WeaverAssetsBuildLocation = "..\\..\\WeaverAssets\\Bundles";
		...
		static void AdjustMonoScripts(string newAssemblyName)
		{
			...
			IEnumerable<string> assetFolderPaths = AssetDatabase.GetAllAssetPaths().Where(path => path.EndsWith(".cs") && path.Contains("Assets/WeaverCore/Hollow Knight"));
		...
		public static void BeginCompile()
		{
				...
				for (int modeIndex = 0; modeIndex < buildModes.Count; modeIndex++)
				{
					...
					if (IsPlatformSupportLoaded(mode.Target))
					{
						...
						foreach (var bundleFile in bundleBuilds.GetFiles())
						{
							...
							{
								File.Copy(bundleFile.FullName, weaverAssetsFolder + "\\weavercore" + mode.Extension, true);
							}
						}
					}
				}
			...
			{
				AdjustMonoScripts("HollowKnight");
			}
		}
