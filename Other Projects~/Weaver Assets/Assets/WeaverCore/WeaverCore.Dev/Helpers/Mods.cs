    ...
    public static class Mods
	{
        public static List<Type> GetMods()
		{
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    if (typeof(WeaverMod).IsAssignableFrom(type) && !type.IsAbstract && !type.IsGenericTypeDefinition && !type.IsInterface && !typeof(WeaverCore.Internal.WeaverCore).IsAssignableFrom(type))
                    {
                        mods.Add(type);
                    }
                }
