...
namespace WeaverCore.Editor
{
	...
	{
		/*class ScriptAssemblyPatch : IPatch
		{
			public void Patch(HarmonyPatcher patcher)
			{
				Debug.Log("PATCHER IS RUNNING---------------------------");
				var assembly = typeof(EditorWindow).Assembly;
				var scriptAssembly = assembly.GetType("UnityEditor.Scripting.ScriptCompilation.EditorCompilation");
				patcher.Patch(scriptAssembly.GetMethod("CreateScriptAssembly"), null, typeof(ScriptAssemblyPatch).GetMethod("Postfix"));
			}
			...
			public static void Postfix(ref object __result)
			{
				try
				{
					...
					Debug.Log("Result Type = " + type);

					var prop = type.GetProperty("References", BindingFlags.Public | BindingFlags.Instance);
					string[] references = (string[])prop.GetValue(__result, null);
					foreach (var reference in references)
					{
						Debug.Log("_-_-_-_Reference = " + reference);
					}
				}
				catch (Exception e)
				{
					Debug.LogError("PATCH ERROR = " + e);
				}
			}
		}*/
