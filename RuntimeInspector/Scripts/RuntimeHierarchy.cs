		...
		public bool ExposeUnityScenes
		{
			...
			set
			{
				if( m_exposeUnityScenes != value )
				{
					...
					{
						if( value )
							OnSceneLoaded(UnityEngine.SceneManagement.SceneManager.GetSceneAt( i ), LoadSceneMode.Single );
						else
							OnSceneUnloaded(UnityEngine.SceneManagement.SceneManager.GetSceneAt( i ) );
					}
				}
			}
		}
