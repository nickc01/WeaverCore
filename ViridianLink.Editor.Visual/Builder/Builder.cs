...
namespace ViridianLink.Editor.Visual
{
	...
	{
		static Assembly ResourceEmbedder;
		...
		static string SelectSaveLocation(string filename)
		{
			...
			{
				buildFolder = new DirectoryInfo("ViridianLink/bin").FullName;
			}
		...
		{
			var EditorDirectory = new DirectoryInfo("Assets\\ViridianLink").FullName;
		...
		static void LoadResourceEmbedder()
		{
			if (ResourceEmbedder == null)
			{
				...
				ResourceEmbedder = AppDomain.CurrentDomain.Load("ResourceEmbedder");
				//Find the main program type
				var program = ResourceEmbedder.GetType("ResourceEmbedder.Program");
		...
		{
			List<Type> ValidMods = Assembly.Load("Assembly-CSharp").GetTypes().Where(type => typeof(IViridianMod).IsAssignableFrom(type) && !type.IsAbstract && !type.IsGenericTypeDefinition && !type.IsInterface && !typeof(ViridianLinkMod).IsAssignableFrom(type)).ToList();
			AdjustMonoScripts();
			LoadResourceEmbedder();
			...
			{
				//var instance = FormatterServices.GetUninitializedObject(modType) as IViridianMod;
				var instance = Activator.CreateInstance(modType) as IViridianMod;
