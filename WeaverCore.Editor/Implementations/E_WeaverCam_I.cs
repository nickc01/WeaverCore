...
{
	public class E_WeaverCam_I : WeaverCam_I
	{
		public class E_Statics : Statics
		{
			public override WeaverCam Create()
			{
				...
				{
					WeaverCam camera = null;
					if ((camera = Camera.main.GetComponent<WeaverCam>()) == null)
					{
						camera = Camera.main.gameObject.AddComponent<WeaverCam>();
					}
					return camera;
				}
			}
		...
		public class E_Shaker : Shaker_I
		{
			Vector3 NeutralPosition;

			Vector3 RumbleAmount = default;
			Coroutine Shaker;
			...
			IEnumerator RumbleRoutine()
			{
				while (true)
				{
					if (RumbleAmount != default && !shaking)
					{
